...
function buildChampionsCupPools(userClub: string): RugbyGroup[] {
  const others = shuffleArray(CHAMPIONS_CUP_CLUBS.filter(c => c !== userClub))
...
function poolFinisher(groups: RugbyGroup[], pool: string, pos: number): string {
  const sorted = sortTable(
    groups.find(g => g.name === pool)!.table
  )
...
export default function ChampionsCup({ state, onUpdate, onExit }: Props) {
  ...
  }, [last16Draw, userClub, userRatings, clubSquads, groups, proceedToQuarterFinalDraw])
  ...
  const fastSimulateFromKnockout = useCallback(
    (
      ...
      const neutral =
        roundKey === 'semi-final' || roundKey === 'final'
  ...
  if (phase === 'results') {
    ...
    const topScorer = getTopTryScorer(userClub, tryScorerCandidates, allResults)
