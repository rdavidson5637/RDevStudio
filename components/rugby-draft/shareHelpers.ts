...
export function getKnockoutUserAwards(
  ...
  const milestoneAwards: Record<string, ShareAward> = {
    ...
    "Pool Stage": {
      label: "Eliminated in Pool Stage",
      icon: "📋",
      tone: "muted",
    },
  };
...
export function getTryScorerCandidates(
  ...
  return slots
    .filter((s) => s.player && TRY_SCORER_POSITIONS.includes(s.player.position))
    .map((s) => ({
      name: s.player!.name,
...
export const SHARE_PLAY_URL =
  process.env.NEXT_PUBLIC_GAME_URL?.trim() || `${SITE_URL}/rugby-draft`;
