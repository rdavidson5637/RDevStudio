...
export const POSITION_COORDINATES: Record<
  Formation,
  { x: number; y: number }[]
> = {
  "4-3-3": [
...
export function getEligiblePlayers(
  ...
  return squad.players.filter((player) => {
    ...
    const compatible = COMPATIBLE_POSITIONS[player.position as Position] ?? [
      player.position,
    ];
...
export function calculateTeamRatings(slots: DraftSlot[]): TeamRatings {
  ...
  const getGroupAvg = (positions: Position[]) => {
    const group = drafted.filter((p) =>
      positions.includes(p.position as Position),
    );
    if (group.length === 0) return 0;
    return Math.round(
      group.reduce((sum, p) => sum + p.overall, 0) / group.length,
    );
  };
...
export function simulateMatch(
  ...
  let homeScore = samplePoissonGoals(
    Math.max(0.4, 1.15 + diff * 0.035 + homeAttackEdge),
  );
  let awayScore = samplePoissonGoals(
    Math.max(0.4, 1.05 - diff * 0.035 + awayAttackEdge),
  );
  ...
  return table.map((row) => {
    if (row.club === result.homeTeam) {
      ...
      return {
        ...
        goalDifference:
          row.goalDifference + (result.homeScore - result.awayScore),
    ...
    if (row.club === result.awayTeam) {
      ...
      return {
        ...
        goalDifference:
          row.goalDifference + (result.awayScore - result.homeScore),
        points: row.points + (won ? 3 : drawn ? 1 : 0),
      };
    }
    return row;
