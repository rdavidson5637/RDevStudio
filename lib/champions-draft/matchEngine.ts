...
export function getSquadRatings(squad: Squad): TeamRatings {
  const draftSlots = squad.players.map((p, i) => ({
    position: p.position,
    player: { ...p, club: squad.club, season: squad.season, league: squad.league },
    coordinates: { x: 0, y: 0 },
  }))
...
export function getTopScorer(
  ...
  const goalsPerGame = userGoals / Math.max(1, allResults.filter(
...
export function buildCLLeaguePhase(
  ...
  const clubs = shuffleArray(clubSquads.map(s => s.club)).slice(0, opponentCount)
...
export function getCLDirectQualifiers(
  table: LeagueTableRow[]
): string[] {
  return table.slice(0, 8).map(r => r.club)
}

export function getCLPlayoffTeams(
  table: LeagueTableRow[]
): string[] {
...
export function simulateKnockoutRoundFromPairings(
  ...
  for (const pairing of pairings.fixtures) {
    ...
    if (home === userTeam || away === userTeam) {
      ...
      const oppRatings = oppSquad
        ? getSquadRatings(oppSquad)
        : { attack: 82, midfield: 82, defence: 82, goalkeeper: 82, overall: 82 }
