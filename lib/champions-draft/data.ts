...
import type { Squad, League, GameMode, WCDraftMode } from '@/types/champions-draft'

const modernData = modernSquadsRaw as { squads: Squad[] }
const historicalData = historicalSquadsRaw as { squads: Squad[] }
const additionalSquadsData = additionalSquadsRaw as { additional_squads: Squad[] }
const additionalNationsData = additionalNationsRaw as { additional_nations: Squad[] }
...
export const ALL_SQUADS: Squad[] = [...ALL_HISTORICAL_SQUADS, ...ALL_MODERN_SQUADS]
...
export const CHAMPIONS_LEAGUE_CLUBS: Squad[] = ALL_MODERN_SQUADS.filter(
  s => ['Premier League', 'La Liga', 'Bundesliga', 'Serie A', 'Ligue 1'].includes(s.league)
)
...
export const NATIONAL_PLAYER_COUNTS: Record<string, number> = Object.fromEntries(
