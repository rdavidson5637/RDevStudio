...
export const RUGBY_POSITIONS: Position[] = [
  'LH', 'HK', 'TH', 'LL', 'RL', 'BF', 'OF', 'N8',
  'SH', 'FH', 'IC', 'OC', 'LW', 'RW', 'FB',
]
...
export function calculateTeamRatings(slots: DraftSlot[]): TeamRatings {
  ...
  const getGroupRating = (positions: Position[], statFn: (player: Player) => number) => {
    const group = drafted.filter(p => positions.includes(p.position))
    if (group.length === 0) return 0
    return Math.round(group.reduce((sum, p) => sum + statFn(p), 0) / group.length)
  }
  ...
  const overall = forwards === 0 && backs === 0
    ? 0
  ...
  return table.map(row => {
    if (row.club === result.homeTeam) {
      ...
      return {
        ...
        pointsDifference: row.pointsDifference + (result.homeScore - result.awayScore),
        tries: row.tries + homeTries,
        bonusPoints: row.bonusPoints + calculateBonusPoints(
          homeTries,
          result.homeScore,
          result.awayScore,
          won,
    ...
    if (row.club === result.awayTeam) {
      ...
      return {
        ...
        pointsDifference: row.pointsDifference + (result.awayScore - result.homeScore),
