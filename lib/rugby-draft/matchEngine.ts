...
const DEFAULT_HOME_RATINGS: TeamRatings = { forwards: 80, backs: 80, overall: 80 }
const DEFAULT_AWAY_RATINGS: TeamRatings = { forwards: 78, backs: 78, overall: 78 }
...
export function generateKnockoutOpponentResult(
  ...
  const homeRatings = homeSquad ? getSquadRatings(homeSquad) : DEFAULT_HOME_RATINGS
  const awayRatings = awaySquad ? getSquadRatings(awaySquad) : DEFAULT_AWAY_RATINGS

  return simulateKnockoutMatch(homeRatings, awayRatings, home, away, neutralVenue)
}
...
export function simulateFullSeason(
  ...
  fixtures.forEach(fixture => {
    ...
    if (fixture.home === userTeam || fixture.away === userTeam) {
      ...
      const oppRatings = oppSquad ? getSquadRatings(oppSquad) : DEFAULT_AWAY_RATINGS
      result = simulateFullMatch(userRatings, oppRatings, userTeam, opponent, isHome)
    } else {
      result = generateOpponentVsOpponentResult(fixture.home, fixture.away, leagueSquads)
    }
...
export function buildKnockoutPairings(
  ...
  for (let i = 0; i < paired.length; i += 2) {
    ...
    fixtures.push({ homeTeam: home, awayTeam: away, homeScore: 0, awayScore: 0 })
  }
...
export function simulateKnockoutRoundFromPairings(
  ...
  for (const pairing of pairings.fixtures) {
    ...
    if (home === userTeam || away === userTeam) {
      ...
      const oppRatings = oppSquad ? getSquadRatings(oppSquad) : DEFAULT_HOME_RATINGS
      ...
      result = generateKnockoutOpponentResult(home, away, allSquads, neutralVenue)
    }
...
export function simulateKnockoutRound(
  ...
  return simulateKnockoutRoundFromPairings(userTeam, userRatings, pairings, allSquads)
}
...
export function buildGroupStage(
  ...
  const otherSquads = shuffleArray(allSquads.filter(s => s.club !== userTeam))
...
export function getTopTryScorer(
  ...
  const userTries = allResults
    ...
    .reduce((sum, r) => {
      return sum + (r.homeTeam === userTeam ? (r.homeTries ?? 0) : (r.awayTries ?? 0))
    }, 0)
